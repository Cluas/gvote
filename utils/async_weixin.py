...
class AsyncWeixinPay:
    ...
    async def _fetch(self, url, data, use_cert=False):
        data.setdefault("appid", self.app_id)
        data.setdefault("mch_id", self.mch_id)
        data.setdefault("nonce_str", self.nonce_str)
        data.setdefault("sign", self.sign(data))

        body = self.to_xml(data)

        if use_cert:
            resp = await self.http_client.fetch(url, method='POST', body=body, cert=(self.cert, self.key))
        else:
            resp = await self.http_client.fetch(url, method='POST', body=body)
    ...
    async def unified_order(self, **data):
        ...
        if "spbill_create_ip" not in data:
            data.setdefault("spbill_create_ip", "")

        raw = await self._fetch(url, data)
        return raw
    ...
    async def refund(self, **data):
        ...
        await self._fetch(url, data, True)
...
async_weixin_pay = AsyncWeixinPay(app_id='wx45973a96444c3cf2',
                                  mch_id='1508958411',
                                  mch_key='qazxswedcvfrtgbnhyujmkiolppowdwq',
                                  notify_url='http://test.cluas.me',
                                  key=BASE_DIR + '/certs/apiclient_key.pem',
                                  cert=BASE_DIR + '/certs/apiclient_cert.pem'
                                  )
